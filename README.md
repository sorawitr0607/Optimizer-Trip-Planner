...
4. **Evidence (`/trips/:id/evidence`)**: Verify opening hours, exact coordinates, admission fees, and transit requirements. Where a free source records a place as closed, the card and the finished itinerary say so — a record to check rather than a verdict, since the source is out of date in both directions — and "This place is gone" is the answer for the cases no source knows. Metro and timetable legs are free and offline, and now count as evidence on a scheduled plan as well as an exploratory one — so a cross-city hop is planned as the ride it is rather than as an hour's walk.
5. **Optimize (`/trips/:id/optimize`)**: Deterministic constraint-aware solver computing daily visit orders and route pacing. Build choices render before their evidence read finishes, and the progress bar counts the remaining wait down. Places are **spread across the trip's days** rather than packed into as few as possible — an otherwise cheaper arrangement, since each day the plan opens costs another journey out from the base and back. Whatever a draft is waiting for — comfort figures to agree to, estimated legs to accept, days to add — is listed together and applied in **one** rebuild. A place no length of trip could hold, or one that still does not fit after days were added for it, is recommended for removal instead of being offered another day. The resulting timetable is grouped by day, each collapsed to a line giving the date, the places on it and the hours it runs.
...
- **Hosted structure recovery**: The hosted `public` schema is archived under
  `supabase/backups/` (snapshot taken 2026-08-28, Supabase era; the live host is now
...
- `supabase/backups/` — schema-only snapshot of the hosted structure (2026-08-28),
  checksum, refresh command, and empty-database recovery procedure.
