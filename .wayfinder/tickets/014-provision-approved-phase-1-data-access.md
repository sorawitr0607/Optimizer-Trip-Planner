...
- Added a standard-library-only [redacted capability check](../../scripts/check_provider_access.py). Normal mode never calls a paid endpoint; `--live-paid` makes one explicit Google Places request only after a key is configured.
...
After the keys are saved, run `python3 scripts/check_provider_access.py` for the no-cost/redacted status and use `--live-paid` only for the final two-request Google verification.
