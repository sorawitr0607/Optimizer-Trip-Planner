...
- Still open. Generated task titles and consequences are English regardless of interface language, because the templates hold literal text rather than codes the way optimizer reasons do; a Thai reader sees Thai labels around English task wording. Destination-specific requirements, transit-country tasks, and the mandatory 30-day, 7-day, and 24-hour verification runs need official sources that no configured provider supplies, so the board can only prompt for them.
