...
| **S4** | **The expensive journey screens.** `places` (570 lines, nearly all-new) · `itinerary` (the six row types, the stop list, the coordinate map) | **Done 2026-08-04.** [Evidence](../../artifacts/validation/2026-08-04-slice-4/notes.md): 312 Python + 26 web tests and all 8 check stages green; the copied real Taipei record walked cached discovery (832 candidates) → ranking → three valid provisional variants → activation → a 69-row shared snapshot → valid XLSX and ICS with no paid call. **The browser witness was obtained the same day** once the connector became available: six retained captures across `places` and `itinerary` in both languages, the tile-free numbered map with its duplicated stop list, and the two `GET` downloads verified over the socket with a bare `GET` to a mutation still refused. **Gate 1 is now assessable.** |
| **S5** | **Slice 6 and the rest.** Non-AI quick actions · `revise` · `readiness` | **Done 2026-08-04.** [Evidence](../../artifacts/validation/2026-08-04-slice-5/notes.md): 328 Python + 38 web tests and all 8 check stages green; *reduce walking* rebuilt a variant showing changed days, a seven-dimension before/after table and moved, removed and displaced stops all **named**; a revision applies and restores on a verified-hours fixture, while the browser witnessed the blocked path because every variant on the real trip is provisional; the board generated 13 items and applies only after previewing. **The S6 deletion checklist is now empty** — all 14 portable behaviours are asserted below Streamlit. Free-text GenAI was deferred at closure and its opt-in React surface was delivered on 2026-08-11. |
...
**2026-08-11 update:** constrained GenAI revision is no longer deferred. `/revise` exposes it behind
