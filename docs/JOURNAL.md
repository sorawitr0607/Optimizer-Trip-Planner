...
Dated build history for this project, moved out of `CLAUDE.md` on 2026-08-21 so the
instruction file stays the size of an instruction file. **Nothing here is current
guidance.** Every entry records what was true on its own date; where a later entry
contradicts an earlier one, the later one wins, and the durable rules that came out of
all of it live in `CLAUDE.md` under "Rules that bind new work".
...
**Superseded the next day.** It does deploy: `api/rpc.py` is the serverless entry, `vercel.json` pins `sin1`, and `psycopg` moved into `[project] dependencies`. See "Running on Vercel" below. Kept because the reasoning about the worker still holds — it is long-lived and Vercel has no home for it.
...
## Phase 2 implementation, slice by slice (WF-MAP-002, 49 of 49 closed)

**Historic.** This records completed work. The rules it established are in `CLAUDE.md`;
the counts in it (allowlist size, refusal codes, dependency count, route count) were
accurate when written and several have since drifted — trust the code, not these numbers.
